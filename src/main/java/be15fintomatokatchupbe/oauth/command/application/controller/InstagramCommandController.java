...
public class InstagramCommandController {
    ...
    @GetMapping("/callback")
    public ResponseEntity<Void> handleInstagramCallback(
...
//        String frontendRedirectBaseUrl = "http://localhost:5173/oauth2/instagram/callback";
        ...
        try {
            instagramTokenService.exchangeCodeForToken(code, influencerId);
            ...
            redirectUrl = UriComponentsBuilder.fromUriString(frontendRedirectBaseUrl)
                    ...
                    .build()
                    .toUriString();
            log.error("인스타그램 계정 연동 실패 (BusinessException): {}", e.getMessage());
        } catch (Exception e) {
            ...
            redirectUrl = UriComponentsBuilder.fromUriString(frontendRedirectBaseUrl)
                    ...
                    .build()
                    .toUriString();
            log.error("인스타그램 계정 연동 실패 (알 수 없는 오류): {}", e.getMessage(), e);
        }
